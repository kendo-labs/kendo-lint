...
function build_kendo(options) {
    ...
    selection.forEach(function(filename){
        ...
        filename = PATH.join(options.kendo_dir, "js", filename);
        var code = FS.readFileSync(filename, { encoding: "utf8" });
...
function kendo_category(dir) {
    try {
        FS.statSync(PATH.join(dir, "js", "kendo.all.min.js"));
        return "all";
    } catch(ex) {}

    try {
        FS.statSync(PATH.join(dir, "js", "kendo.web.min.js"));
        return "web";
    } catch(ex) {}

    try {
        FS.statSync(PATH.join(dir, "js", "kendo.mobile.min.js"));
        return "mobile";
    } catch(ex) {}

    try {
        FS.statSync(PATH.join(dir, "js", "kendo.dataviz.min.js"));
        return "dataviz";
    } catch(ex) {}
}
